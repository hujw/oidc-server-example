...
const debug = new Debug('oidc-provider:eduinfo');
...
module.exports = function eduinfoAction(provider) {
  const {
    pairwiseSalt,
    claims: claimsConfig,
    claimsSupported,
  ...
  return [
    ...
    async function loadAccount(ctx, next) {
		//hujw
      const account = await provider.Eduinfo.findByEduId(
        ctx,
    ...
    async function respond(ctx, next) {
      const claims = _.get(ctx.oidc.accessToken, 'claims.eduinfo', {});
      ...
      if (client.userinfoSignedResponseAlg || client.userinfoEncryptedResponseAlg) {
        ...
        const token = new IdToken(
          await ctx.oidc.account.claims('eduinfo', scope, claims, rejected),
