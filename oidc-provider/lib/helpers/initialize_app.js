...
const {
  getAuthorization, getUserinfo, getEduinfo, getToken, getCertificates, getRegistration, getRevocation,
...
module.exports = function initializeApp() {
  ...
  //hujw
  const eduinfo = getEduinfo(this);
  const eduInfoCors = getCors({
    allowedMethods: 'GET,POST', exposeHeaders: 'WWW-Authenticate', allowHeaders: 'Authorization',
  });
  
  get('eduinfo', routes.eduinfo, eduInfoCors, error(this, 'eduinfo.error'), ...eduinfo);
  post('eduinfo', routes.eduinfo, eduInfoCors, error(this, 'eduinfo.error'), ...eduinfo);
  options('eduinfo', routes.eduinfo, eduInfoCors);
